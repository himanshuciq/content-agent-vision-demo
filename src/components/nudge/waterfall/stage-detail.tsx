...
export function StageDetail({ stage }: { stage: WaterfallStage }) {
  ...
  return (
    <div className="mt-6 overflow-hidden rounded-xl border border-slate-200">
      <div className="flex items-center justify-between gap-4 border-b border-slate-100 bg-slate-25 px-6 py-4">
        <div className="flex items-center gap-3">
          <span className="text-lg font-semibold text-slate-950">{stage.label}</span>
          <span className="font-mono text-base font-bold text-slate-950 tabular-nums">{fmtM(stage.value)}</span>
        </div>
        {stage.canNudgeTeam &&
          (teamNudged ? (
