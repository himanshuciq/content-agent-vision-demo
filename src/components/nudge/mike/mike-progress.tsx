...
import { BATCHES, DEADLINE, OPEN_BY_AREA } from "../data"
...
const CONTENT_OPEN = money(OPEN_BY_AREA.find((a) => a.agent === "content")!.value)
...
export function MikeProgress({ celebrate }: { celebrate: Celebrate | null }) {
  ...
  const open = CONTENT_OPEN - doneValue
  const whole = BANKED + CONTENT_OPEN

  return (
    <>
      <div className="px-10 pt-4 pb-7">
        <div className="text-2xl font-semibold tracking-tight text-slate-700">
          <span className="font-mono text-slate-950">{fmt(open)}</span> of content opportunity is {doneValue > 0 ? "still " : ""}open this quarter.
        </div>
      ...
      <div className="relative border-y border-slate-200 bg-brand-25 px-10 py-4">
          ...
          <div className="text-sm text-slate-600 tabular-nums">
            ...
            {doneValue > 0 && (
              <>
                ...
                <span className="font-semibold text-brand-700">{fmt(doneValue)} approved</span>
              </>
            )}
          </div>
        ...
        <div className="mt-2.5 flex h-2 gap-0.5 overflow-hidden rounded-full bg-brand-100">
          ...
          <div className="h-full bg-brand-400 transition-[width] duration-500 ease-out" style={{ width: `${(doneValue / whole) * 100}%` }} />
        </div>
      </div>
    </>
  )
